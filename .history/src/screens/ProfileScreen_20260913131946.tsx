...
export default function ProfileScreen() {
  ...
  const handlePetExclusion = (pet: any) => {
    Alert.alert('Excluir pet', `Deseja realmente excluir ${pet?.nome || 'este pet'}?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Excluir',
        style: 'destructive',
        onPress: async () => {
          try {
            ...
            // Aqui você pode adicionar a invalidação da query se necessário (ex: queryClient.invalidateQueries(['pets']))
          } catch (error) {
  ...
  const handlePetEdit = (pet: any) => {
    // Lógica para editar o pet (ex: navegar para a tela de cadastro/edição passando o pet)
          ...
          {pets.length > 0 ? (
            <View style={styles.petsListContainer}>
              {pets.map((pet: Pet, index: number) => (
                <View key={pet.idPet || index} style={index > 0 && { marginTop: 12 }}>
                  <PetCard
                    pet={pet}
                    onEdit={() => handlePetEdit(pet)}
                    onDelete={() => handlePetExclusion(pet)}
                  />
                </View>
              ))}
            </View>
