...
const API_UPDATE_URL = process.env.EXPO_PUBLIC_API_JAVA_USER;
...
// Adicione em eloPetService.js
export const getUserService = async (id) => {
  try {
    const config = await getAuthConfig();
    const response = await axios.get(`${API_UPDATE_URL}/${id}`, config);
    return response.data; // Retorna os dados completos (nome, email, telefone, etc.)
  } catch (error) {
    console.error('Erro ao buscar dados do usuário:', error);
    throw new Error(error.response?.data?.message || 'Erro ao carregar dados do perfil.');
  }
};
