...
import { createPetService, getStoredUserId } from '../services/eloPetService';
...
export default function RegisterPetScreen({ navigation }: { navigation?: any }) {
  ...
  const [flagCastrado, setFlagCastrado] = useState<number>(0); // 0 = Não, 1 = Sim
  ...
  const handleRegisterPet = async () => {
    ...
    try {
      ...
      // Payload no mesmo formato esperado pelo Postman: { idResponsavel, pet: { ... } }
      const payload = {
        ...
        pet: {
          ...
          sexo: gender,
          flagCastrado: flagCastrado,
        },
      };

      await createPetService(payload);
            ...
            <View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}>
              ...
              <View style={styles.speciesRow}>
                {[
                  ...
                  const isSelected = flagCastrado === item.value;
                  return (
                    <TouchableOpacity
                      key={item.label}
                      activeOpacity={0.8}
                      ...
                      onPress={() => setFlagCastrado(item.value)}
                    >
                      <Text
                        style={[
                          styles.speciesButtonText,
