...
import { getPetsService, getStoredUserId, updateUserService } from '../services/eloPetService';
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (user) {
      setName(user.nomeCompleto || 'Usuário');
      setEmail(user.email || '');
  ...
  const handleGetPets = async () => {
    ...
    const { data: pets, isError } = useQuery({
      queryKey: ['pets'],
        ...
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.petCard}>
            ...
            <View style={styles.petInfo}>
              <Text style={styles.petName}>{pets?.[0]?.nome}</Text>
              <Text style={styles.petDetails}>{pets?.[0]?.raca} · Código {pets?.[0]?.idadeAproximada}</Text>
            </View>
