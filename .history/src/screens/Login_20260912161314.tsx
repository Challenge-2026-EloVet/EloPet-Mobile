...
export default function Login() {
  ...
  try {
    ...
    // 1. Faz o login e obtém o retorno (ex: { token: '...', idUsuario: 1 })
    const loginResponse = await loginService(username, password);
    
    // Extrai o idUsuario independentemente de vir direto ou dentro de um objeto
    const userId = loginResponse.idUsuario || loginResponse.id || loginResponse.user?.idUsuario;

    if (userId) {
      // 2. Busca os dados completos do usuário usando a API /responsible/{id}
      const userData = await getUserService(userId);
      if (setUser) setUser(userData);
    } else {
      // Fallback caso o idUsuario não venha no login (embora você tenha dito que vem)
      if (setUser) setUser({ nomeUsuario: username });
    }
