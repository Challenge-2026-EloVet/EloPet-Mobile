...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (user) {
      setName(user.nomeCompleto || user.nomeUsuario || 'Usuário');
      setEmail(user.usuario?.email || user.email || '');
  ...
  const { data: rawPets = [] } = useQuery({
    queryKey: ['pets', userId],
    queryFn: () => getPetsService(userId),
    enabled: !!userId,
  });

  const pets = Array.isArray(rawPets)
    ? rawPets.map((item) => (item?.pet ? { ...item.pet, idPetResponsavel: item.idPetResponsavel } : item))
    : [];
  ...
  const handlePetExclusion = (pet: any) => {
    Alert.alert('Excluir pet', `Deseja realmente excluir ${pet?.nome || 'este pet'} da sua lista de cuidados?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await deletePetService(pet.idPet);
          } catch (error) {
  ...
  // AQUI EU TIVE QUE MUDAR A ESTRUTURA DA PAGINA DO PADRÃO QUE USAMOS PQ ESTAVA CRASHANDO O SWIPELISTVIEW DO PETCARD
  // COM O SCROLLVIEW DA PROFILESCREEN, ENTAO CRIEI ESTRUTURA DE HEADER E FOOTER PARA O PETCARD, E COLOQUEI O SCROLLVIEW DENTRO DO PETCARD, ASSIM ELE CONSEGUE RENDERIZAR OS PETS COM O SWIPELISTVIEW SEM CRASHAR
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <PetCard
        pets={pets}
        onEdit={(pet: Pet) => handlePetEdit(pet)}
        onDelete={(pet: Pet) => handlePetExclusion(pet)}
        ListHeaderComponent={
          <>
