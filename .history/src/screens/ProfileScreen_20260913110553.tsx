...
export default function ProfileScreen() {
  ...
  // Ampliado para abranger qualquer variação de ID retornada pelo backend Java
  ...
  // Sincroniza os estados locais sempre que o objeto 'user' do AuthContext mudar/carregar
  useEffect(() => {
    if (user) {
      setName(user.nomeUsuario || user.nomeCompleto || user.nome || 'Usuário');
      setEmail(user.email || '');
      setPhone(user.telefone || user.phone || '');
      setCity(user.cidade || user.city || '');
    }
  }, [user]);
  ...
  // Resto do código da tela permanece igual...
