...
export default function ProfileScreen() {
  ...
  const userData = user?.userId || user?.nomeUsuario || user?.nomeCompleto || 'Usuário';
  const userId = user?.idResponsavel || user?._id || user?.userId
  ...
  const [name, setName] = useState(userData?.nomeUsuario || 'Usuário');
  const [email, setEmail] = useState(userData?.ema || '');
  const [phone, setPhone] = useState(user?.telefone || '');
  const [city, setCity] = useState(user?.cidade || '');
