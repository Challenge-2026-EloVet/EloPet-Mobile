...
import { getPetsService, updateUserService } from '../services/eloPetService';
...
export default function ProfileScreen() {
  ...
  const { data: rawPets = [], isLoading: isLoadingPets } = useQuery({
  queryKey: ['pets', userId],
  queryFn: () => getPetsService(userId),
  enabled: !!userId,
});

// Transforma [{ pet: { nome: 'Mel', ... } }, ...] em [{ nome: 'Mel', ... }, ...]
const pets = Array.isArray(rawPets)
  ...
  useEffect(() => {
    if (pets && pets.length > 0) {
      setPetName(pets[0].nome);
      setPetAge(pets[0].idadeAproximada);
      setPetBreed(pets[0].raca);
      setPetSpecies(pets[0].especie);
      setPetGender(pets[0].sexo);
      setPetBirthDate(pets[0].dataNascimento);
      setPetCastrated(!!pets[0].flagCastrado);
    }
  }, [pets]);
        ...
        <View style={styles.sectionContainer}>
          ...
          {pets.length > 0 ? (
            pets.map((pet: Pet, index: number) => (
              <View key={pet.idPet || index} style={[styles.petCard, index > 0 && { marginTop: 10 }]}>
                ...
                <View style={styles.petInfo}>
                  ...
                  <Text style={styles.petDetails}>
                    {pet?.raca} · Idade: {pet?.idadeAproximada || 'Não informada'}
                  </Text>
                </View>
