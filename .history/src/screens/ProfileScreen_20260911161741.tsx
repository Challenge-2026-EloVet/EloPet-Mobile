...
import { updateUserService } from '../services/upuserService'; // Ajuste o caminho conforme o seu projeto

export default function ProfileScreen() {
  const route = useRoute<any>();
  const userFromParams: any = route.params?.user || {};

  // Pegamos o ID do usuário (suportando 'id', '_id' ou 'userId')
  const userId = userFromParams.id || userFromParams._id || userFromParams.userId;

  const [isEditing, setIsEditing] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  // Estados dos campos do formulário preenchidos com os dados recebidos
  ...
  const [phone, setPhone] = useState(userFromParams.telefone || userFromParams.phone || '');
