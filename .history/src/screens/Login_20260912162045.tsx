...
export default function Login() {
 ...
 const handleLogin = async () => {
    try {
      ...
      // 1. Faz o login e obtém o token e o idUsuario
      const loginResponse = await loginService(e, password);
      
      const idUsuario = loginResponse.idUsuario || loginResponse.id;

      if (!idUsuario) {
        throw new Error('A API de login não retornou o idUsuario.');
      }

      // 2. Executa a requisição GET na API /responsible/{idUsuario} para buscar os dados cadastrais
      const userData = await getUserService(idUsuario);

      // 3. Grava os dados completos no Contexto e libera a navegação
        ...
        >
          ...
          <View style={styles.footerContainer}>
            <Text style={styles.footerText}>
              {activeTab ? 'Já tiene uma conta? ' : 'Ainda não tem uma conta? '}
            </Text>
