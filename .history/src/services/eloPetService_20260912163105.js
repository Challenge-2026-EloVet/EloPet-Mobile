...
export const loginService = async (email, senha) => {
  try {
    ...
    const idUsuario = response.data.idUsuario || response.data.id;

    if (token) {
      await storage.setItem(TOKEN_KEY, String(token));
    }
    if (idUsuario) {
      await storage.setItem(USER_ID_KEY, String(idUsuario));
    }
...
export const registerService = async (nomeUsuario, email, senha, tipoUsuario = 'RESPONSAVEL', nomeCompleto) => {
  try {
